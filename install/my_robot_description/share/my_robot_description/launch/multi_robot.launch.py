...
def generate_launch_description():
    ...
    model2_arg = DeclareLaunchArgument(
        name="model2",
        default_value=os.path.join(get_package_share_directory("my_robot_description"), "urdf", "robot_description.urdf.xacro"),
        description="Absolute path to robot 2nd URDF file"
    )
...
######
    robot2_description = ParameterValue(Command(["xacro ", LaunchConfiguration("model2")]), value_type=str)
#####
...
#######
    ...
    spawn_robot_1 = Node(
        ...
        arguments=["-entity", "bumperbot_1", "-topic", "robot_description"],
        output="screen"
    )
    ...
    spawn_robot_2 = Node(
        package="gazebo_ros",
        executable="spawn_entity.py",
        arguments=["-entity", "bumperbot_2", "-topic", "robot2_description"],
        output="screen"
    )
